...
using Xunit;
using InternalDi = KF.RestApi.Internal.Sample.DependencyInjection.ServiceCollectionExtensions;

namespace KF.RestApi.Architecture.ApiModuleSeed.Tests;
...
public sealed class LayeringRules
{
    ...
    public void DomainDependsOnExternal()
    {
        var result = Types.InAssembly(typeof(ISampleService).Assembly)
            .That().AreClasses()
            .And().ResideInNamespace("KF.RestApi.Domain.Sample.Services")
            .Should().HaveDependencyOn("KF.RestApi.External.Sample")
            .GetResult();
    ...
    public void InternalDoesNotReferenceExternal()
    {
        var result = Types.InAssembly(typeof(InternalDi).Assembly)
            .Should().NotHaveDependencyOn("KF.RestApi.External.Sample")
            .GetResult();
