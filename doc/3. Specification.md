...
>
> * The `KF.RestApi.External.*`, `KF.RestApi.Domain.*`, `KF.RestApi.Internal.*`, and `KF.RestApi.Client.*` libraries
...
All names use a placeholder **`<ApiName>`** – e.g. `KF.RestApi.External.Foo`, `KF.RestApi.Domain.Foo`, etc.
...
/ApiGatewayFramework
  /src
    /KF.RestApi.KF.RestApi.Common.Abstractions
    /KF.RestApi.KF.RestApi.Common.Observability
    /KF.RestApi.KF.RestApi.Common.Persistence
    /KF.RestApi.KF.RestApi.Common.Analyzers
    /KF.RestApi.External.<ApiName>
    /KF.RestApi.Domain.<ApiName>
    /KF.RestApi.Internal.<ApiName>
    /KF.RestApi.Client.<ApiName>
    /KF.RestApi.Host.Internal
  /templates
    /dotnet-new-external
    /dotnet-new-domain
    /dotnet-new-internal
...
* `openapi.json` in `KF.RestApi.External.<ApiName>/openapi/openapi.json`, or
* `openapi.yaml` in `KF.RestApi.External.<ApiName>/openapi/openapi.yaml`.
...
Generated code lives under `KF.RestApi.External.<ApiName>`:

* All **DTOs and Refit interfaces are `internal`**.
* Namespace: `KF.RestApi.External.<ApiName>.Generated`.
...
`KF.RestApi.External.<ApiName>` exposes a single extension method:

```csharp
namespace KF.RestApi.External.<ApiName>;

using KF.RestApi.External.<ApiName>.Generated;
...
## 3. Domain Layer Contract (`KF.RestApi.Domain.<ApiName>`)
...
```csharp
namespace KF.RestApi.Domain.<ApiName>.Models;
...
```csharp
namespace KF.RestApi.Domain.<ApiName>.UseCases;
...
* `I<ApiName>RefitClient` (from `KF.RestApi.External.<ApiName>`)
* `IApiAuditRepository` (from `KF.RestApi.KF.RestApi.Common.Persistence`)
* Logging and tracing abstractions from `KF.RestApi.KF.RestApi.Common.Observability`.

```csharp
namespace KF.RestApi.Domain.<ApiName>.UseCases;

using KF.RestApi.Common.Observability;
...
```csharp
namespace KF.RestApi.Domain.<ApiName>;

using KF.RestApi.Domain.<ApiName>.UseCases;
using Microsoft.Extensions.DependencyInjection;
...
## 4. Internal Layer Contract (`KF.RestApi.Internal.<ApiName>`)
...
```csharp
namespace KF.RestApi.Internal.<ApiName>;
...
`KF.RestApi.Host.Internal` is the only **real host**:
...
## 5. Client Layer Contract (`KF.RestApi.Client.<ApiName>`)
...
```csharp
namespace KF.RestApi.Client.<ApiName>;
...
All audit persistence lives in `KF.RestApi.KF.RestApi.Common.Persistence`.
...
## 7. Roslyn Analyzer Suite (`KF.RestApi.KF.RestApi.Common.Analyzers`)
...
* **Message:** `Type '{0}' in namespace '{1}' must be declared 'internal' in KF.RestApi.External.* projects.`
* **Description:** Enforces that all publicly exposed types in `KF.RestApi.External.*` are internal to prevent leaking provider DTOs outside the boundary.
...
{
    const string source = @"
namespace KF.RestApi.External.Foo;
...
<Project>
  ...
  <ItemGroup>
    ...
    <PackageReference Include="KF.RestApi.KF.RestApi.Common.Analyzers" Version="1.0.0" PrivateAssets="all" />
  </ItemGroup>
</Project>
```
...
### 8.3 Example `.csproj` – `KF.RestApi.External.<ApiName>`

```xml
<Project Sdk="Microsoft.NET.Sdk">

  <PropertyGroup>
    <RootNamespace>KF.RestApi.External.<ApiName></RootNamespace>
...
**File:** `/tools/KF.RestApi.NginxConfigGen/config/apis.json`
...
**C# model in `KF.RestApi.NginxConfigGen`:**
...
/templates/dotnet-new-external
  ...
  /KF.RestApi.External.<ApiName>.csproj
...
* References to `KF.RestApi.Common.*` packages
...
* Steps:
  ...
  6. Generate Internal OpenAPI spec from `KF.RestApi.Host.Internal` (for docs).
...
jobs:
  build-test:
    ...
    steps:
      ...
      - run: dotnet run --project ./tools/KF.RestApi.NginxConfigGen/KF.RestApi.NginxConfigGen.csproj
```
...
* Packs and publishes:
  ...
  * `KF.RestApi.KF.RestApi.Common.Persistence`
  * `KF.RestApi.KF.RestApi.Common.Analyzers`
  * `KF.RestApi.Client.*` (if you decide they are reusable libs)
    ...
    steps:
      ...
      - uses: actions/setup-dotnet@v4
        with:
          dotnet-version: '9.0.x'
      - run: dotnet restore
      - run: dotnet pack src/KF.RestApi.KF.RestApi.Common.Abstractions/KF.RestApi.KF.RestApi.Common.Abstractions.csproj -c Release -o ./artifacts
      - run: dotnet pack src/KF.RestApi.KF.RestApi.Common.Analyzers/KF.RestApi.KF.RestApi.Common.Analyzers.csproj -c Release -o ./artifacts
      - name: Push packages
